...
from ..features.slater_magnetic import TorusMagneticSections
...
class MagneticBoundaryAdapter(nn.Module):
    ...
    def __init__(
        ...
        if not isinstance(coefficients, torch.Tensor) or coefficients.ndim != 1:
            raise TypeError("base model must expose one-dimensional determinant coefficients c")
    ...
    def c(self) -> torch.Tensor:
        """Return the wrapped model's determinant coefficients."""
    ...
    def set_gate_multiplier(self, value: float) -> None:
        """Forward HMGA gate warmup while remaining a no-op for references."""
